...
# ╔═╡ 2ca72906-3b25-4046-b1d6-39fd3ceed499
md"""

* Our system is governed by a incredibly ugly set of differential equations
...
* We are going to be using the Angular equations. 

* Working in an ideal physical system, so energy should be conserved

* Finding lyapunov exponents. These quantify how fast two very close initial conditions diverge from each other. We will find one for set of initial conditions.  This is given by 


$\sigma = \lim_{n\to\infty}\frac{1}{n}\left\{\sum_{i=0}^{n-1}\ln\big|\dot{\omega}_{(t_i)}\big|\right\}$
"""
...
md"""
For our model, we will be using the following equations which were derived from *Chen, J. (2008)*:
...
$\dot{\omega_{2}} = \frac{m_{2}l_{2}\omega_{2}^{2}\sin(2\Delta\theta)+2(m_{1}+m_{2})l_{1}\omega_{1}^{2}+2g(m_{1}+m_{2})\cos(\theta_{1})\sin(\Delta\theta)}{2l_{2}(m_{1}+m_{2}\sin^{2}(\Delta\theta))}$
Where $m$ are the masses of the pendulum, $l$ are the lengths of the pendulum, and $g$ is the acceleration due to gravity. For our project, we used both masses as $1\;kg$ and both lengths as $1\;m$ for simplicity.

The packages that we use in this project are DifferentialEquations.jl, ChaosTools.jl, and FLoops for the calculations.

The code below is the model of the double pendulum.
"""

# ╔═╡ ef5ff5d9-1ccd-4a91-b0cb-2c0fb4070e53
begin
	function pendulum(dth, th, para, t) #This is the pendulum function
		...
		ds = th[1] - th[2] #This sets up Δθ
		
		dth[3] = (m2*l1*((th[3])^2)*sin(2*ds)+2*m2*l2*((th[4])^2)*sin(ds)+2g*m2*cos(th[2])*sin(ds)+2*g*m1*sin(th[1])) / (-2*l1*(m1+(m2*sin(ds)^2))) #This is the time derivative of ω_1
		dth[4] = (m2*l2*((th[4])^2)*sin(2*ds)+2*(m1+m2)*l1*((th[3])^2)*sin(ds)+2*g*(m1+m2)*cos(th[1])*sin(ds)) / (2*l2*(m1+(m2*(sin(ds))^2))) #This is the time derivative of ω_2
	end
end

# ╔═╡ 61128297-d130-4237-9e2f-48f20778bd61
md"""
The next two cells below setup a few variables. The first cell sets up time related variables and the system's parameters.
"""

# ╔═╡ a0e34738-5bf5-4ba3-aa34-cf0b71cfbf2a
begin
	p = [1 1 1 1 9.81] #This sets up our parameters in the order of m1, m2, l1, l2, and g
	
	#This sets up the time stuff as well as the potential algorithms
	ti = 0
	tf = 30
	tstep = 10000
...
# ╔═╡ 1cdbbdd6-4d65-46c7-a985-f04230482fc1
md"""
This cell here sets up $3$ different initial conditions. A regular system $(0.25\pi,0.25\pi)$, a chaotic system $(\pi,\pi)$, and a system where we don't know what it could be $(2.139,1.592)$.
"""
...
md"""
This cell here solves problems for given initial conditions and algorithms. Before we continue with looking at our $3$ scenarios, we need to determine the best algorithm to use.
"""

# ╔═╡ b933fcf0-ab70-46d0-9508-f42c1ea86dab
begin
...
# ╔═╡ e9e24d47-af03-49d4-8ba5-b4d7c00fbfa9
md"""
This cell here solves the problems using Tsit5.
"""

# ╔═╡ b3a36c34-dbb1-4056-86a5-9e6c6797f502
begin
...
# ╔═╡ 5c4a4c65-6041-4782-b55e-3da026581f39
md"""
This one here uses TanYam7 to sovle the problems.
"""

# ╔═╡ 0154a287-cd6c-4f6f-9814-6c79ab668c25
begin
...
# ╔═╡ 73a183b7-6d15-4889-9aee-e68a005fcf6d
md"""
Here we are using Vern6.
"""

# ╔═╡ 2c470c38-1555-4517-90e0-46bf60092e59
begin
...
# ╔═╡ dc0fdc3f-d9dd-4686-9fab-809cf58a8837
md"""
Finally, we are using Midpoint to solve the problems in this cell.
"""

# ╔═╡ 4bd9021f-c727-4b7b-89a4-4e90abae4693
begin
...
# ╔═╡ d4e990c7-d468-43f0-85fc-7c7ba4a4531e
md"""
The cell below creates solution sets. So for example, all of the regular solutions are gathered up.
"""

# ╔═╡ e532713b-d92f-4d44-9538-56170a258c39
begin
...
# ╔═╡ 3dbcc0f2-573b-440e-ac69-a05f51f6f836
md"""
Now we can determine which algorithm to use. To see which one we want to use, we can calculate the change in energy and try to find the one that has the overall smallest change as our system *should* conserve energy. The cell below is a function that allows us to calculate the change in energy for a given solution.
"""

# ╔═╡ 3676a75d-9e72-48f4-bb97-d9b23e75c6fe
begin
	function energy_change(solution)
		#The four variable before are for the two angles and two angular velocities
		th1 = solution[1,:]
		th2 = solution[2,:]
		w1 = solution[3,:]
		w2 = solution[4,:]
		
		n = length(solution[1,:]) #This gives us a length
		E = zeros(n) #This sets up a vector
		i = 1 #This sets up our counter
		while i <= tstep
			T1 = 0.5*p[1]*(p[3]*sin(w1[i])).^2 #This is the kinetic energy of the first pendulum
			T2 = T1 + 0.5*p[2]*(p[4]*sin(w2[i])).^2 #This is the kinetic energy of the second pendulum
			U1 = p[1]*p[5]*p[3]*cos(th1[i]) #This is the potential energy of the first pendulum
			U2 = U1 + p[2]*p[5]*p[4]*cos(th2[i]) #This is the potential energy of the second pendulum
		...
		E_delta = zeros(n) #This sets up a vector
...
md"""
The cell below will take $4$ solutions and then determine the change in energy for all $4$. It will then plot the change in energy and depending on the tuple that is parsed as the second argument, will either make it into $1$ plot or into $4$ smaller ones.
"""
...
md"""
Now we can restrict the angle solutions between -π and π.
"""

# ╔═╡ 4d4af392-6f98-428e-9c8b-5ed771c0b0a4
begin
...
# ╔═╡ 05a1b656-04ec-4fd7-9d9a-1eedcdcd50ae
md"""
The code below groups the angle solutions and the angular velocities solutions together.
"""

# ╔═╡ 48eebf67-d3a5-4861-a17c-5727cc83773f
begin
...
# ╔═╡ 48b33849-93b7-4e5a-aeac-774239a1b325
md"""
Now we can plot the angle and angular velocity over time. This will be seen down in the results. The code below is for the animations.
"""

# ╔═╡ dbb7fbd5-4601-43e5-afcc-5c99bfe15381
begin #Everything here is to make the animation quicker. We don't use the tempo time step and range for the heat maps as the accuracy there is more valuable there than it is here.
	tempo_tstep = 1000
	tempo_tr = LinRange(ti,tf,tempo_tstep)
end

# ╔═╡ 409514d2-93bd-408f-9eff-1777b4f59ee9
begin
	#This is the regular zone
	tempo_solved_regular = problem_solver(s_regular, alg[3], tempo_tr)

	#This is the chaotic zone
	tempo_solved_chaotic = problem_solver(s_chaotic, alg[3], tempo_tr)
	
	#This is for the mystery area
	tempo_solved_mystery = problem_solver(s_mystery, alg[3], tempo_tr)
end

# ╔═╡ 3f2afd4c-5159-4f6f-95a8-a4f10ea374b8
md"""
The cell below contains the animation solutions.
"""

# ╔═╡ 9961ddea-c729-42af-85fb-93f8b8c592fe
begin
	gif_regular = tempo_solved_regular[1:2,:]
	gif_chaotic = tempo_solved_chaotic[1:2,:]
	gif_stable = tempo_solved_mystery[1:2,:]
end

# ╔═╡ 334e4aa6-7ded-4e79-a2d8-fe1c9dc1ee7e
md"""
The function below allows us to plot what the pendulum looks like at a given time.
"""

# ╔═╡ a8c614f3-e8fd-4461-b17e-db8147e15945
begin
...
# ╔═╡ 674b81e4-fa42-4f92-ae8b-f93d8b3ff2b9
md"""
The function below allows us to make an animation for a particular solution. Those animations will be in the **Results** section.
"""

# ╔═╡ dc326ea1-6ada-45c5-a14e-508dc08452f6
begin
	function animation(gif)
		anim = @animate for i in eachindex(tempo_tr)
			gif_maker(gif[:,i,], tempo_tr[i], p[3], p[4])
		end
	end
end

# ╔═╡ 4742cd56-ad7d-4327-bab9-c16f0e1c12b7
md"""
The code below allows us to find the lyapunov exponent for a given initial condition and time period.
"""

# ╔═╡ c2b2dc86-a675-4a38-b3e5-e964c3424a40
begin
...
md"""
Now using the function lya(), we can determine a matrix of lyapunov exponents over the range of -π and π for both angles.
"""
...
begin
	...
	@floop for i in amount, j in amount
		...
		punov = lya(condition, tf, tstep, alg[2]) #This finds the Lyapunov exponent (punov is to differentate it from the exponent variable in lya, but it is the same thing)
...
# ╔═╡ b5a26e98-94cc-4a51-b922-5c350f24ab66
md"""
$(Resource("https://github.com/NeonKarearea/Chaos-Control/blob/main/pendComparison5.gif?raw=true"))
"""
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "ae4c5f325247f8f96641bcef3827d8aa82ffaa01"
