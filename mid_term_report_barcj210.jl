...
using OrdinaryDiffEq, ChaosTools, Plots

# ╔═╡ 52716ab2-d273-11ed-1f86-09f8d71d4725
md"""
# Mid-term Report: CJ Barnes-Wilkie (Student ID: 3580179)"""
...
# ╔═╡ ec9b1795-59bc-454c-aeb5-e063eff3d58a
md"""
Chaos theory is a branch of mathematics that stems from the 1600's when Newton developed differential equations. Using his laws of motion and universal gravitation, as well as Kepler's laws of planetary motion, Newton was able to solve a two body system (i.e. the Sun and the Earth), however a problem arises when a third body enters the fray, such as the Sun, Earth, and Moon system. Over the next few centuries several mathematicians and physicists attempted to solve the three body system but none came close, and the problem was deemed impossible.$^{[1]}$ In 1885, a mathematician called Henri Poincaré decided to try solving the problem. Instead of trying to take an analytical approach with three massive bodies, Poincaré broke it down into a simpler system: two massive bodies orbiting around their common center of gravity, and then a miniscule mass which did not affect the orbits of the larger masses. He was able to prove that this is stable. However, when Poincaré wrote the essay for publication, Poincaré found an error in his results. Once he corrected the error, Poincaré found that even a small change to the initial conditions could make large and unpredictable changes in the orbit. The system was chaotic.$^{[2]}$ Not much work on chaos occurred in $20^{th}$ century, until the computer was developed. Once the computer started to take off, analysing chaotic systems became easier and allowed the discovery of chaotic properties such as the Lorenz attractor in 1963.$^{[1]}$

An example of a chaotic system is the logistic map, defined as $x_{n}\to rx_{n}(1-x_{n})$ for $0<r<4$. This *looks* like a nice quadratic equation, but the problem arises when r increases. For small $r$ less than $3$, there is a period of one for small perturbations around $x_{n}$ but once we get to the critical point $r_{2} = 3$ then the system splits into two separate lines that diverge, or bifurcates. This is now period 2 as it takes twice as long to cycle through. This keeps happening as $r$ gets bigger and reaches new critical points. Eventually, we get to $r_{\infty}\approx3.5699$. Trying to pass this point results in a chaotic system.$^{[3, 4]}$
"""

# ╔═╡ 4df03b26-18b8-4303-97aa-2d3d2e15611c
md"""
$(Resource(logistic_map_url))
*Fig. 1: The logistic map.*"""

# ╔═╡ 2f5a0ca2-67d7-4a8d-9283-67c1448e0ad1
md"""
Another property of chaotic systems is the Lyapunov exponent. The definition of the Lyapunov exponent is:

$\sigma=\lim_{n\to \infty}\frac{1}{n}\left\{\sum_{i=0}^{n-1}\ln\big|f'(x_i)\big|\right\}$where $n$ is the number of iterations, and $f'(x_i)$ is the derivative of a given function. For the logistic map, this is $f'(x_n) = 2rx_n-r$. For a negative Lyapunov exponent then the system is regular, but if the exponent is positive then the system is chaotic.$^{[5]}$

This can all be applied to the double pendulum system, as we will delve into more detail in the **Theory** section. Having an understanding of chaos itself allows for understanding of complex systems such as the three body problem, but other examples include heat conduction and turbulence in fluids.$^{[6]}$ Having understanding of the double pendulum allows for better understanding of systems such as limb movement in sports like golf$^{[7]}$ and by extending the model from two pendulums to four, we can model the stability of the mirrors used at the LIGO observatory.$^{[8]}$
"""

# ╔═╡ 8b1de72b-3f09-42e7-aeb7-0b6a3d5a8091
md"""
## Theory
For a single pendulum, we have a mass $m$ in kg and a length $l$ in m. We can release it from an angle $\theta$ in radians and from there we can the position and velocity of the pendulum. We get:
...
where $x$ is the $x$ position, $y$ is the $y$ position, $l$ is the length of the pendulum, $\theta$ is the angle, and the dotted variables are the time derivatives. For a single pendulum, it is easy to predict how it will behave, but what if we have two pendulums with one of them attached to the mass of the other? """

# ╔═╡ efe1e3f0-3bbe-4522-b13e-e0393bf8b431
md"""
$(Resource(double_pendulum_url))
*Fig. 2: Double Pendulum*
"""

# ╔═╡ 325dd681-c79b-4563-be1a-8f51f5c01742
md"""This particular system is much more difficult to solve by hand, as we can see by the position and velocity equations for the double pendulum. Below are the position equations$^{[7]}$.

$x_{1} = l_{1}\sin(\theta_{1})$
$y_{1} = -l_{1}\cos(\theta_{1})$
$x_{2} = x_{1} + l_{2}sin(\theta_{2}) = l_{1}\sin(\theta_{1})+l_{2}\sin(\theta_{2})$
$y_{2} = y_{1} - l_{2}cos(\theta_{2}) = -l_{1}\cos(\theta_{1})-l_{2}\cos(\theta_{2})$where variables with the 1 subscript are for the first pendulum, and variables with the 2 subscript are for the second variable. Below are the velocity equations$^{[7]}$.
...
Another thing we can look at for chaotic systems is the Lyapunov exponential. We can  find this from the $\omega_{(t)}$ function, which will be equal to the sum of both $\omega$ for each part of the pendulum, so$^{[5]}$:

$\delta_0 = \omega_{(t_0 + \delta t)}-\omega_{(t_0)}$From this we can write:

$|\delta| = |\delta_0|e^{n\sigma}$where $\sigma$ is equal to the Lyapunov exponential. The Lyapunov exponential can be determined by adapting the formula from the Chaos-II lecture.

$\sigma = \lim_{n\to\infty}\frac{1}{n}\left\{\sum_{i=0}^{n-1}\ln\big|\dot{\omega}_{(t_i)}\big|\right\}$This allows us to determine if there is chaotic motion.
"""
...
begin
	...
	th1 = 0.25pi #Inital condition of θ_1
	...
	tr = LinRange(ti,tf,1000)
end

# ╔═╡ e0377e47-0377-45fa-981c-eaea93bdf082


# ╔═╡ e80dc8b6-c1a7-4bb8-96c5-8bea54a9bf55
begin
	prob = ODEProblem(pendulum!, s, (ti,tf), p) #This sets up the problem
	solvin = solve(prob,alg,saveat=tr) #This solves it.
end

# ╔═╡ 4612cf35-f36c-4cae-8c45-1c37128349ac
begin
	plot(tr, [solvin[1,:], solvin[2,:]], labels = ["θ1" "θ2"])
end

# ╔═╡ 59492867-d943-4078-8906-c12ddf0bc2c3
begin
	plot(tr, [solvin[3,:], solvin[4,:]], labels = ["ω1" "ω2"])
end

# ╔═╡ b276b408-cec4-4334-a726-15ebc9d3991a
begin
	theta_1 = solvin[1,:]
	theta_2 = solvin[2,:]
	@gif for i in eachindex(tr)
		scatter(theta_1)
		ylims!(-30, 30)
	end
end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "ef0830f4259f44866bc0268ea582dae175b15732"
